...
namespace CityPlanning.Modules
{
    ...
    {
        private bool isFirstTimeControlsizeChanged;
        public ucSpatialAnalysisResult()
        {
            InitializeComponent();
            isFirstTimeControlsizeChanged = true;
        }
        ...
        public ucSpatialAnalysisResult(string originalFilePath, string resultFilePath)
        {
            ...
            if (this.axPageLayoutControl1.CheckMxFile(resultFilePath))
            {
                this.axPageLayoutControl1.LoadMxFile(resultFilePath, "");
            }
        ...
        private void ucSpatialAnalysisResult_SizeChanged(object sender, EventArgs e)
        {
            ...
            if (isFirstTimeControlsizeChanged)
            {
                this.axMapControl1.Extent = this.axMapControl1.FullExtent;
                this.axMapControl2.Extent = this.axMapControl1.Extent;
            }
            isFirstTimeControlsizeChanged = false;
            this.axMapControl1.Refresh();
            this.axMapControl2.Refresh();
        }
